...
class GNB():
    ...
    def train(self, X, Y):
        ...
        # means = [[s, d, s_dot, d_dot],  class1
        #          [s, d, s_dot, d_dot],  class2
        #          [s, d, s_dot, d_dot],  class3
        #          [s, d, s_dot, d_dot]]  class4
        means = []
        stds = []
    ...
    def _predict(self, obs):
        ...
        for (means, stds, lab) in zip(self._means, self._stds, self.classes):
            ...
            for mu, sig, o in zip(means, stds, obs):
                #根据朴素贝叶斯，分别将各参数在该类别下出现的概率相乘
                likelihood = gaussian_prob(o, mu, sig)
                product *= likelihood
            probs.append(product)
        t = sum(probs)
        #根据全概率公式,P(s, d, s_dot, d_dot) = sum(s,d,s_dot,d_dot|class)
